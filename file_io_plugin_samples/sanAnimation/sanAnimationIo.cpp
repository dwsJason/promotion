...
// some useful defines
#define FILE_TYPE_ID "de.cosmigo.fileio.san"
#define FILE_BOX_DESCRIPTION L"SAN - Sample Animation"
#define FILE_EXTENSION L"san"
...
#define FILE_HEADER_TYPE_ID "SANM"
...
// frame index counter for loading saveing
unsigned int currentFrameIndex;

// file handle for reading and writing
FILE* file;

// helper to close global file
void closeFile()
{
	if ( file!=NULL )
	{
...
extern "C"
{
	bool __stdcall initialize( char* language, unsigned short* version, bool* animation )
	{
		...
		*animation= true; // it's an animation processing plugin
	...
	int  __stdcall getImageCount()
	{
		...
		return fileHeader.numberOfFrames;
	}
	...
	bool  __stdcall loadNextImage( unsigned char* colorFrame, unsigned char* colorFramePalette, unsigned char* alphaFrame, unsigned char* alphaFramePalette, unsigned short* delayMs )
	{
		...
		if ( file==NULL )
		{
			file= _wfopen( currentFileName, L"rb" ); 
			if ( file==NULL )
			{
				wcscmp( lastErrorMessage, ERROR_FILE_OPEN_FAILED );
				return false;
			}
			...
			{
				wcscmp( lastErrorMessage, ERROR_FILE_READ_FAILED );
				closeFile();
				return false;
			}
		}
		...
		// read colors
		if ( fread( colorFramePalette, 1, 768, file ) != 768 )
		{
			wcscmp( lastErrorMessage, ERROR_FILE_READ_FAILED );
			closeFile();
			return false;
		...
		{
			if ( fread( alphaFramePalette, 1, 256, file ) != 256 )
			{
				wcscmp( lastErrorMessage, ERROR_FILE_READ_FAILED );
				closeFile();
				return false;
		...
		// set progress
		if ( fileHeader.numberOfFrames > 0 ) 
			updateProgress( 100 * currentFrameIndex / fileHeader.numberOfFrames );
	...
	bool  __stdcall beginWrite( int width, int height, int transparentColor, bool alphaEnabled, int numberOfFrames )
	{
		...
		fileHeader.numberOfFrames= numberOfFrames;

		// reset progress
		updateProgress( 0 );

		// open file for writing
		file= _wfopen( currentFileName, L"w+b" ); 
		if ( file==NULL )
		{
			wcscpy( lastErrorMessage, ERROR_FILE_OPEN_FAILED );
			return false;
